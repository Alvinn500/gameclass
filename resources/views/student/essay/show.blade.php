<x-student-layout title="soal {{$task->title}}">
    ...
    />
    <div class="my-8"> 
        <div class="dark-purple shadow-md rounded-2xl py-4 px-8">
            <div id="start">
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    ...
                    <div class="w-full sm:w-[49%] flex gap-3 justify-center flex-col items-center sm:items-start">
                        <h1 class="font-bold text-2xl md:text-3xl uppercase text-neutral-800">{{$task->title}}</h1>
                        @if($totalEssay === 0)
                            <p class="text-red-500 text-sm font-semibold">Belum ada soal</p>
                        @else
                        <p class="text-base font-semibold text-neutral-800">Jumlah Soal {{$totalEssay}}</p>
                        @endif
                        <div>
                            <button {{$totalEssay === 0 ? 'disabled' : ''}} class="bg-indigo-600 px-10 md:px-14 py-2.5 md:py-3 rounded-lg" onclick="hideshow('start','end')">Mulai quiz</button>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        @foreach ($essays as $essay)  
                            ...
                            <div class="{{$loop->iteration !== 1 ? "mt-6" : "mt-2"}} mb-8 flex flex-col gap-2 space-x-3">
                                ...
                                <textarea 
                                    ...
                                    cols="30" 
                                    rows="10"
                                    class="bg-main text-neutral-800 placeholder:text-neutral-500 rounded-xl border border-neutral-300 text-sm p-4"
                                    placeholder="Tulis Jawaban Nomor {{$loop->iteration}} di sini" 
                                ></textarea>    
                            </div> 
                        @endforeach
                        <button type="submit" class="bg-indigo-600 text-sm font-semibold uppercase py-3 px-8 mb-2 rounded-lg shadow-lg">Kumpulkan jawaban</button>
                    </div>
                </form>
            </div>
        </div>
