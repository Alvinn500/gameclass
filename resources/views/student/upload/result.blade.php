        ...
        <div class="mt-6">
            <div class="dark-green flex flex-col sm:flex-row rounded-xl pb-8 sm:pb-0">
                ...
                <div class="w-full p-4 sm:pr-2 lg:pr-4 flex flex-col justify-center items-center gap-4">
                    ...
                    @else
                        <p class="text-sm sm:text-base font-semibold">kamu mendapatkan reward {{$user->uploadScores->where('user_id', Auth::user()->id)->first()->XP ?? 0}} XP</p>    
                        <div class="bg-yellow-400 inline-block text-center py-5 px-16 sm:px-20 md:px-24 lg:px-28 md:space-y-3 lg:space-y-4">
                            ...
                            <h3 class="font-bold text-3xl md:text-4xl lg:text-5xl text-black">{{$user->uploadScores->where('user_id', Auth::user()->id)->first()->score ?? 0}}</h3>
                        </div>
                        <div class="bg-secondary p-4 w-80">
                            <h2 class="text-yellow-400 font-semibold uppercase">Komentar</h2>
                            <p>{{$user->uploadScores->where('user_id', Auth::user()->id)->first()->comment ?? "-"}}</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="flex flex-col gap-5">
                <div class="p-4 dark-blue rounded-xl space-y-4">
                    ...
                    @endif
                    <a href="{{asset("uploads/" . $user->uploadAnswers->where("upload_id", $task->upload->id)->first()->file)}}" target="_blank" class="bg-violet-800 inline-block text-sm font-semibold uppercase py-3 px-8 mb-2 rounded-lg">lihat jawabanmu</a>
                </div>
            </div>
        </div>
</x-student-layout>