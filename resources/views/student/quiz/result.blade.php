    ...
    <div class="mt-6">
        <div class="dark-green shadow-md flex flex-col sm:flex-row rounded-xl pb-8 sm:pb-0">
            ...
            <div class="w-full p-4 sm:pr-2 lg:pr-4 flex flex-col justify-center items-center gap-4">
                <h1 class="text-neutral-800 font-bold text-center sm:text-left text-lg sm:text-xl md:text-2xl lg:text-3xl">{{$task->type === 1 ? $task->title : "Kamu sudah mengerjakan soal tes $task->title"}}</h1>
                <p class="text-neutral-800 text-sm sm:text-base font-semibold">kamu mendapatkan reward {{$rewerdXP}} xp</p>
                <div class="bg-yellow-400 inline-block text-center py-5 px-16 sm:px-20 md:px-24 lg:px-28 md:space-y-3 lg:space-y-4">
                    ...
                    <h3 class="font-bold text-3xl md:text-4xl lg:text-5xl text-neutral-800">{{$grade}}</h3>
                </div>
                @if($task->type === 2)
                    <p class="text-sm text-center text-neutral-800 sm:text-left">Kunci jawaban tes kamu tidak di tampilkan karena soal bersifat rahasia</p>
                @endif
            </div>
        </div>
        @if($task->type === 1) 
            <h2 class="font-bold text-2xl my-4 uppercase text-neutral-800">hasil quiz</h2>
            <div class="flex flex-col gap-4">
                @foreach ($task->multipleChoices as $quiz)
                    <div class="p-4 bg-main shadow-md rounded-xl space-y-2">
                        ...
                        @endif
                        <h3 class="text-lg font-semibold text-neutral-800">{{$quiz->question}}</h3>
                        <div>
