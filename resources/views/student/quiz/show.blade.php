<x-student-layout title="Soal {{$task->title}}">
    ...
    <div> 
        <div class="bg-main shadow-lg rounded-2xl mb-4 py-4 px-8">
            <div id="start">
                <div class="bg-yellow-500 mx-auto w-[98%]">
                    <h2 class="text-main font-semibold text-sm p-2.5 -mt-6">Rewird Hingga {{$totalQuiz ? 50 * $totalQuiz : 0}} XP</h2>
                </div>
                <div class="flex flex-col sm:flex-row gap-3">
                    ...
                    <div class="w-full sm:w-[49%] flex gap-3 justify-center flex-col items-center sm:items-start">
                        <h1 class="font-bold text-2xl md:text-3xl uppercase text-main">quiz</h1>
                        @if($totalQuiz === 0)
                            <p class="text-red-500 text-sm font-semibold">Belum ada soal</p>
                        @else
                            <p class="text-base font-semibold text-main">Jumlah Soal {{$totalQuiz}}</p>
                        @endif
                        <div>
                            <button {{$totalQuiz === 0 ? 'disabled' : ''}} class="bg-button text-main px-10 md:px-14 py-2.5 md:py-3 rounded-lg" onclick="hideshow('start','Q1')">Mulai quiz</button>                                
                        </div>
                    </div>
                </div>
            </div>
                ...
                <form action="" method="POST">
                    ...
                    @foreach ($quizzes as $quiz)    
                        <div id="Q{{$i}}" style="display: none" class="flex-col items-center sm:flex-row">
                            <div class="w-full sm:w-[50%] flex flex-col gap-8 justify-center items-center">
                                ...
                                @endif
                                <h1 class="font-semibold text-main">{{$quiz->question}}</h1>
                            </div>
                            <div class="w-full sm:w-[50%] flex flex-col gap-3">
                                ...
                                <label for="optA{{$quiz->id}}" class="bg-focus hover:bg-hover py-4 rounded-lg text-center font-semibold shadow-md px-2">{{$quiz->options['a']}}</label>
                                ...
                                <label for="optB{{$quiz->id}}" class="bg-focus hover:bg-hover py-4 rounded-lg text-center font-semibold shadow-md px-2">{{$quiz->options['b']}}</label>
                                ...
                                <label for="optC{{$quiz->id}}" class="bg-focus hover:bg-hover py-4 rounded-lg text-center font-semibold shadow-md px-2">{{$quiz->options['c']}}</label>
                                ...
                                <label for="optD{{$quiz->id}}" class="bg-focus hover:bg-hover py-4 rounded-lg text-center font-semibold shadow-md px-2">{{$quiz->options['d']}}</label>
                                ...
                                <label for="optE{{$quiz->id}}" class="bg-focus hover:bg-hover py-4 rounded-lg text-center font-semibold shadow-md px-2">{{$quiz->options['e']}}</label>
                            </div>
                        </div>
                        @php
                            $i++;
                    ...
                    <div id="end" style="display: none" class="flex-col sm:flex-row">
                        <div class="w-full sm:w-[70%] flex order-2 sm:order-1 flex-col justify-center items-center sm:items-start gap-5 ">
                            <h2 class="text-xl sm:text-2xl md:text-3xl text-center sm:text-left font-bold text-main">YEAY! kamu sudah menyelesaikan quiz</h2>
                            <div>
                                <button type="submit" class="bg-button text-sm font-semibold uppercase py-3 px-8 rounded-lg shadow-lg">simpan dan lihat hasilnya</button>
                            </div>
                        </div>
