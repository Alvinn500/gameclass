<x-student-layout title="Soal {{$task->title}}">
    ...
    <div> 
        <div class="dark-purple shadow-lg rounded-2xl mb-4 py-4 px-8">
            <div id="start">
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    ...
                    <div class="w-full sm:w-[49%] flex gap-3 justify-center flex-col items-center sm:items-start">
                        <h1 class="font-bold text-2xl md:text-3xl uppercase text-neutral-800">quiz</h1>
                        @if($totalQuiz === 0)
                            <p class="text-red-500 text-sm font-semibold">Belum ada soal</p>
                        @else
                            <p class="text-base font-semibold text-neutral-800">Jumlah Soal {{$totalQuiz}}</p>
                        @endif
                        <div>
                            <button {{$totalQuiz === 0 ? 'disabled' : ''}} class="bg-indigo-600 px-10 md:px-14 py-2.5 md:py-3 rounded-lg" onclick="hideshow('start','Q1')">Mulai quiz</button>                                
                ...
                <form action="" method="POST">
                    ...
                    <div id="end" style="display: none" class="flex-col sm:flex-row">
                        <div class="w-full sm:w-[70%] flex order-2 sm:order-1 flex-col justify-center items-center sm:items-start gap-5 ">
                            <h2 class="text-xl sm:text-2xl md:text-3xl text-center sm:text-left font-bold text-neutral-800">YEAY! kamu sudah menyelesaikan quiz</h2>
                            <div>
