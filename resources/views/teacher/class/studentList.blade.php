<x-teacher-layout title="Rekap Nilai">
    ...
    />
    <div>
        <h1 class="my-4 text-sm font-semibold text-main">DAFTAR SISWA</h1>
                    ...
                    @foreach ($users as $user) 
                        @if($user)
                            <tr>
                                ...
                                <td class=" border-b text-main text-sm">{{$user->email}}</td>
                                <td class=" border-b text-main text-sm">{{$user->phone}}</td>
                                <td class=" border-b text-sm relative">
                                    <div class="tooltip absolute hidden bg-focus rounded-xl w-fit -top-14 -left-6 z-50">
                                        <p class="text-main font-semibold text-xs px-4 py-2">Keluarkan Dari Kelas</p>
                                    </div>
                                    <button id="deleteUser">
                                        <img id="item" class="w-8 cursor-pointer mx-auto" src="{{asset('image/delete-user.png')}}" alt="delete student icon">
                                    </button>
                        ...
                        <div id="confirm_overlay" class="fixed hidden justify-center items-center z-50 bg-black bg-opacity-45 w-full h-full top-0 left-0">
                            <div id="confirm_modal" class="bg-main rounded-lg p-4 w-96 flex flex-col gap-4">
