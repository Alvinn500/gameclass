<x-teacher-layout title="{{$class->study_name }} - {{$class->class}}">
    ...
    />
    <div class="bg-neutral-900 mt-6 px-6 pt-2 pb-6 rounded-xl">
        <form action="" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            @method("PATCH")
            <div class="flex flex-col gap-2">
                <label for="study_name" class="text-sm">Nama Mata Pelajaran</label>
                <input type="text" name="study_name" id="study_name" value="{{$class->study_name}}" class="bg-neutral-900 border border-neutral-700 text-sm p-2.5 rounded-lg">
                <x-form-error name="study_name"/>
            </div>
            <div class="flex flex-col gap-2">
                <label for="school_name" class="text-sm">Nama Sekolah</label>
                <input type="text" name="school_name" id="school_name" value="{{$class->school_name}}" class="bg-neutral-900 border border-neutral-700 text-sm p-2.5 rounded-lg">
                <x-form-error name="school_name"/>
            </div>
            <div class="flex flex-col gap-2">
                <label for="class" class="text-sm">Kelas</label>
                <input type="text" name="class" id="class" value="{{$class->class}}" class="bg-neutral-900 border border-neutral-700 text-sm p-2.5 rounded-lg">
                <x-form-error name="class"/>
            </div>
            <div class="flex flex-col gap-2">
                <label for="logo_class" class="text-sm">Logo Kelas (.jpg/.png)</label>
                @if($class->logo_class)
                    <img class="w-16 rounded-lg" src="{{asset("logo_class/" . $class->logo_class)}}" alt="logo_class">
                @endif
                <input type="file" name="logo_class" id="logo_class" class="bg-neutral-900 border border-neutral-700 text-sm p-2.5 rounded-lg">
            </div>
    ...
    <div id="confirm_overlay" class="fixed hidden top-0 left-0 w-full h-full z-50 bg-black bg-opacity-50">
        <div id="confirm_modal" class="flex justify-center items-center h-full">
            <form action="" method="POST" id="deleteForm" class="bg-neutral-900 rounded-lg p-4 w-96 flex flex-col gap-4">
                @csrf
                @method("DELETE")
                <div>
                    <h2 class="font-semibold text-lg text-center mb-1">Hapus Kelas</h2>
