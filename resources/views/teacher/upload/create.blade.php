<x-teacher-layout title="{{$task->title}}">
    ...
    <div>
        <div class="dark-purple shadow-md rounded-xl flex gap-2 px-3 py-4 my-4">
            <button id="editUploadTask" class="uppercase block text-xs px-4 py-3 rounded-lg font-bold bg-indigo-600">edit judul Tugas</button>
        </div>
        ...
        <div id="modalEditUpload" class="fixed z-50 hidden justify-center items-center w-full h-screen bg-black bg-opacity-70 top-0 left-0 ">
            <div class="bg-main rounded-xl w-2/3 sm:w-2/5 lg:w-1/3 inline-block">
                <form id="editForm" action="/upload/edit/{{$task->id}}" method="POST" class="flex flex-col p-4 gap-4">
                    ...
                    <input id="inputTask" class="bg-main focus:outline-none border border-neutral-300 text-neutral-800 placeholder:text-neutral-500 font-medium rounded-lg p-2.5 text-sm" type="text"  name="title" value="{{$task->title}}" placeholder="Masukan judul task">
        ...
        <div class="flex flex-col md:flex-row gap-4 mb-4">
            @if(!$upload)
                <div class="bg-main shadow-md px-6 py-8 space-y-4 rounded-2xl w-full md:w-[70%] lg:w-[75%]">
                    <h2 class="text-neutral-500">Anda belum membuat pertanyaan</h2>
    ...
    <div id="parent_upload_overlay" class="fixed hidden z-50 bg-black bg-opacity-70 w-full h-full top-0 left-0 overflow-scroll">
        <div id="upload_overlay" class="flex h-full justify-center items-center py-6">
            <form action="" method="POST" enctype="multipart/form-data" class="bg-main p-4 rounded-xl w-[80%] md:w-[60%]">
                @csrf
                <div class="flex justify-between">
                    <h2 class="text-sm md:text-base font-semibold text-neutral-800">Buat Tugas</h2>
