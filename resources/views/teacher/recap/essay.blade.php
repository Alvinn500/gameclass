<x-teacher-layout title="Rekap Nilai">
    ...
    />
    <div>
        <h1 class="my-4 text-sm font-semibold text-main">REKAP NILAI ESSAY {{$essay->title}}</h1>
                    ...
                    @foreach ($users as $user) 
                        @if($user->essayScores && !$user->essayScores->where("task_id", $essay->id)->isEmpty())
                            <tr>
                                ...
                                <td class="{{$user->essayScores->where("task_id", $essay->id)->first()->status ? "text-link" : "text-red-500"}} border-b text-sm">{{$user->essayScores->where("task_id", $essay->id)->first()->status ? "Sudah dinilai" : "Belum dinilai"}}</td>
                                <td class="border-b text-base"><a class="uppercase text-xs font-semibold bg-button py-2 px-6 rounded-md" href="/teacher/recap/{{$user->id}}/{{$essay->id}}/essay/answer">Lihat jawaban</a></td>
                            </tr>
                        @endif
                    @endforeach
                </tbody>
