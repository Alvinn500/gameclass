<x-teacher-layout title="Rekap Nilai">
    ...
    <x-teacher-sub-nav 
        classPath="/teacher/{{$class->id}}/{{$lesson->id}}/{{$game->id}}/essay/create" 
    ...
    />
    <div>
        <h1 class="my-4 text-sm font-semibold">REKAP NILAI GAME {{$game->title}}</h1>
                    ...
                    @foreach ($users as $user) 
                        @if($user->memoryGameScores && !$user->memoryGameScores->where("task_id", $game->id)->isEmpty())
                            <tr>
                                ...
                                <td class="border border-neutral-800 text-base">{{$user->memoryGameScores->where("task_id", $game->id)->first()->score}}</td>
                                <td class="border border-neutral-800 text-base">{{$user->memoryGameScores->where("task_id", $game->id)->first()->created_at->setTimezone('Asia/Jakarta')->format('d M Y (H:i T)')}}</td>
                                <td class="border border-neutral-800 text-base">{{$user->memoryGameScores->where("task_id", $game->id)->first()->XP}} XP</td>
                                <td class="{{$user->memoryGameScores->where("task_id", $game->id)->first()->status ? "text-lime-500" : "text-red-500"}} border border-neutral-800 text-sm">{{$user->memoryGameScores->where("task_id", $game->id)->first()->status ? "Sudah dinilai" : "Belum dinilai"}}</td>
                                <td class="border border-neutral-800"><a class="uppercase text-xs font-semibold bg-lime-500 py-2 px-6 rounded-md" href="/teacher/recap/{{$user->id}}/{{$game->id}}/game/answer">Lihat jawaban</a></td>
                            </tr>
                        @endif
                    @endforeach
                </tbody>
