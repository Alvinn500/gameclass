<x-teacher-layout title="Rekap Nilai">
    ...
    />
    <div>
        <h1 class="my-4 text-sm font-semibold text-neutral-800">REKAP NILAI GAME {{$game->title}}</h1>
                    ...
                    @foreach ($users as $user) 
                        @if($user->memoryGameScores && !$user->memoryGameScores->where("task_id", $game->id)->isEmpty())
                            <tr>
                                ...
                                <td class="{{$user->memoryGameScores->where("task_id", $game->id)->first()->status ? "text-lime-500" : "text-red-500"}} border-b text-sm">{{$user->memoryGameScores->where("task_id", $game->id)->first()->status ? "Sudah dinilai" : "Belum dinilai"}}</td>
                                <td class="border-b text-base"><a class="uppercase text-xs font-semibold bg-indigo-600 text-white py-2 px-6 rounded-md" href="/teacher/recap/{{$user->id}}/{{$game->id}}/game/answer">Lihat jawaban</a></td>
                            </tr>
                        @endif
                    @endforeach
                </tbody>
