    ...
    <div>
        ...
        <div class="flex gap-6 min-w-full p-4 rounded-xl bg-neutral-900">
            <div class="flex flex-col gap-1">
                ...
                <h2>Nilai</h2>
                <h2>Komentar</h2>
                <button id="updateScore" class="bg-violet-800 text-sm font-semibold uppercase py-2 px-4 rounded-lg mt-2">{{$user->essayScores->where("task_id", $essay->id)->first()->status ? "ubah nilai" : "beri nilai"}}</button>
            </div>
            <div class="flex flex-col gap-1">
                <p>: {{$essay->title}}</p>
                <p>: {{$user->name}}</p>
                <p>: {{$user->essayScores->where("task_id", $essay->id)->first()->created_at->setTimezone('Asia/Jakarta')->format('d M Y (H:i T)')}}</p>
                <p>: {{$XP}} XP</p>
                <p>: {{$score}}</p>
                <p>: {{$user->essayScores->where("task_id", $essay->id)->first()->comment ?? "-"}}</p>
            </div>
        </div>
        <h2 class="my-4 text-sm font-semibold">HASIL JAWABAN {{$essay->title}}</h2>
