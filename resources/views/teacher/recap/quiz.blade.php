<x-teacher-layout title="Rekap Nilai">
    ...
    />
    <div>
        <h1 class="my-4 text-sm font-semibold text-main">{{$quiz->type === 1 ? "REKAP NILAI QUIZ $quiz->title" : "REKAP NILAI TEST $quiz->title"}}</h1>
                    ...
                    @foreach ($users as $user) 
                        @if(!$user->multipleChoiceAnswers->where("task_id", $quiz->id)->isEmpty())
                            <tr>
                                ...
                                <td class="border-b text-base"><a class="uppercase text-xs font-semibold bg-button text-main py-2 px-6 rounded-md" href="/teacher/recap/{{$user->id}}/{{$quiz->id}}/quiz/answer">Lihat jawaban</a></td>
                            </tr>
                        @endif
                    @endforeach
                </tbody>
