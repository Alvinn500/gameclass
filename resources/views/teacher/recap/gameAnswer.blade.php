    ...
    <div>
        ...
        <div class="flex gap-6 min-w-full p-4 rounded-xl bg-main shadow-md">
            <div class="flex flex-col gap-1">
                ...
                <button id="updateScore" class="bg-button text-sm font-semibold uppercase py-2 px-4 rounded-lg mt-2">{{$user->memoryGameScores->where("task_id", $game->id)->first()->status ? "ubah nilai" : "beri nilai"}}</button>
            </div>
            <div class="flex flex-col gap-1">
                <p class="text-main font-medium">: {{$game->title}}</p>
                <p class="text-main font-medium">: {{$user->name}}</p>
                <p class="text-main font-medium">: {{$user->memoryGameScores->where("task_id", $game->id)->first()->created_at->setTimezone('Asia/Jakarta')->format('d M Y (H:i T)')}}</p>
                <p class="text-main font-medium">: {{$XP}} XP</p>
                <p class="text-main font-medium">: {{$score}}</p>
                <p class="text-main font-medium">: {{$user->memoryGameScores->where("task_id", $game->id)->first()->comment ?? "-"}}</p>
            </div>
        </div>
            ...
            @foreach (array_slice($memory->images, 0, 6) as $key => $image)
                <div class="p-4 bg-main shadow-md rounded-xl space-y-3">
                    ...
                    <div class="bg-focus rounded-lg p-4">
                        <p class="text-main font-semibold">{{$memory->answers->where('user_id', $user->id)->first()->answers[$key]}}</p>
                    </div>
                </div>
            @endforeach
        </div>
