<x-teacher-layout title="{{$task->title}}">
    ...
    <div>
        ...
        <div id="modalEditTask" class="fixed z-50 hidden justify-center items-center w-full h-screen bg-black bg-opacity-70 top-0 left-0 ">
            <div class="bg-main rounded-xl w-2/3 sm:w-2/5 lg:w-1/3 inline-block">
                <form id="editForm" action="/task/edit/{{$task->id}}" method="POST" class="flex flex-col p-4 gap-4">
                    ...
                    <input id="inputTask" class="bg-input focus:outline-none font-medium text-input placeholder:text-placehodler rounded-lg p-2.5 text-sm" type="text"  name="title" value="{{$task->title}}" placeholder="Masukan judul task">
        ...
        <div class="flex flex-col md:flex-row gap-4">
            @if(!$memory)
                <div class="bg-main shadow-md px-6 py-8 rounded-2xl w-full md:w-[70%] lg:w-[75%]">
                    <div id="emptyGame" class="space-y-4">
                        <h2 class="text-secondary">Anda belum membuat game</h2>
                        <span class="block h-[0.0625rem] mx-auto w-full bg-line"></span>
                        <x-form-error name="images"/>
                        <x-form-error name="images.*"/>
                        <button id="addGame" class="uppercase block text-xs px-4 py-3 rounded-lg font-bold text-main bg-yellow-400">tambah pertanyaan</button>
                    </div>        
                    <div id="addImageGame" class="hidden">
                        <form action="" method="POST" enctype="multipart/form-data">
                           @csrf
                           <h2 class="text-sm md:text-base font-semibold text-main">Tambahkan 6 Gambar</h2>
                           <span class="block h-[0.0625rem] mx-auto w-full bg-line my-4"></span>
                           <div class="flex items-center gap-2">
                               <div class="flex flex-col gap-2 mb-1 lg:mb-0">
                                   <lable for="image" class="text-xs lg:text-sm font-semibold text-main">Gambar</lable>
                                   <div class="flex flex-col sm:flex-row items-center gap-2">
                                       <input type="file" name="images[]" multiple id="image" class="w-full py-2.5 px-3 bg-input text-input rounded-xl text-sm">
                                       <button type="submit" class="uppercase px-8 py-1.5 font-semibold rounded-lg w-full lg:w-auto block text-xs bg-button">Tambah Pertanyaan</button>
                                   </div>
                                   <x-form-error name="images"/>
                               </div>
                           </div>
                ...
                <div class="order-2 md:order-1 w-full px-4 py-5 rounded-2xl mb-6 bg-main shadow-md md:w-[70%] lg:w-[75%] space-y-2">
                    @if(!$memory->questions)
                        <div class="flex flex-col gap-3 justify-between">
                            <h1 class="font-medium text-sm text-main">Buat pertanyaan masing masing gambar <span class="text-link">(contoh: Apa fungsi dari gambar tersebut)</span></h1>
                            <form action="" method="POST" class="flex flex-col gap-4">
                                @csrf
                                @if($memory)
                                    @foreach (array_slice($memory->images, 0, 6) as $key => $image)
                                        <div class="flex flex-col gap-2">
                                            <div class="w-[40%] flex justify-center items-center bg-focus p-8 rounded-lg">
                                                <img class="w-[60%] rounded-lg" src="{{asset("$image")}}" alt="image memory game {{$key}}">
                                            </div>
                                            <input type="text" name="question{{$key}}" id="question{{$key}}" value="{{old("question$key")}}" placeholder="Apa fungsi dari gambar di atas?" class="bg-input text-input focus:outline-none p-2 text-sm rounded-md placeholder:text-placehodler">
                                        </div>
                                ...
                                <div class="flex flex-col gap-2">
                                    ...
                                    <div class="flex items-center gap-4 p-2 border border-main rounded-md w-28">
                                        <img class="w-6" src="{{asset("image/time.png")}}" alt="">
                                        <input type="text" name="time" id="time" value="00:00" placeholder="00:00" pattern="[0-9]{2}:[0-9]{2}" class="bg-main text-main focus:outline-none text-sm placeholder:text-placehodler w-full"> 
                                    </div>
                                </div>
                                <button type="submit" class="bg-button text-sm font-semibold uppercase py-2 rounded-lg">Tambah Pertanyaan</button>
                            </form>
                        </div>
                    @endif
                    @if($memory->questions)
                        <div class="flex flex-col gap-3 justify-between">
                            ...
                            <div class="grid grid-cols-2 gap-6">
                                @if($memory)
                                    @foreach (array_slice($memory->images, 0, 6) as $key => $image)
                                        <div class="flex flex-col gap-2">
                                            <div class="w-[75%] mx-auto flex justify-center items-center bg-focus p-8 rounded-lg">
                                                <img class="w-[60%] rounded-lg" src="{{asset("$image")}}" alt="image memory game {{$key}}">
                                            </div>
                                            <p class="text-sm text-center font-medium text-main"><span class="font-semibold text-link">Pertanyaan:</span> {{$memory->questions[$key]}}</p>
                                        </div>
                                    @endforeach
                                @endif  
                            </div>
                            <div class="flex flex-col gap-2 mt-6">
                                <h2 class="text-sm font-medium text-main">Waktu pengerjakan?</h2>
                                <div class="flex items-center gap-4 border border-main p-2 rounded-md w-28">
                                    <img class="w-6" src="{{asset("image/time.png")}}" alt="">
                                    <P class="text-main">{{$memory->time}}</P>
                                </div>
                            </div>
            ...
            <div class="order-1 md:order-2 w-full md:w-[30%] lg:w-[25%] h-fit bg-main shadow-md rounded-2xl p-4">
                <h2 class="font-bold mb-2 text-4xl lg:text-5xl text-main">{{$studentAnswered}}</h2>
                <p class="text-sm mb-4 text-main">Siswa telah menyelesaikan game</p>
        ...
        <div id="modal_edit_challenge" class="flex justify-center items-center py-6">
            <form action="/game/images/edit/{{$task->id}}" method="POST" enctype="multipart/form-data" class="bg-main p-4 rounded-xl w-[80%] md:w-[60%]">
                ...
                <div class="flex flex-col gap-6">
                    <div class="flex flex-col gap-2 mb-1 lg:mb-0">
                        <lable for="image" class="text-xs lg:text-sm font-medium text-main">Ganti gambar</lable>
                        <input type="file" name="images[]" multiple id="image" class="py-2.5 px-3 text-input bg-input placeholder:text-placehodler rounded-xl text-sm">
                        <x-form-error name="images"/>
                    </div>
                    @if($memory)
                       @foreach (array_slice($memory->images, 0, 6) as $key => $image)
                           <div class="flex flex-col gap-3">
                               ...
                               <input type="text" name="question{{$key}}" id="question{{$key}}" value="{{$memory->questions[$key] ?? ''}}" placeholder="Apa fungsi dari gambar di atas?" class="bg-input text-input focus:outline-none p-2 text-sm rounded-md placeholder:text-placehodler">
                           </div>
                           <x-form-error name="question{{$key}}"/>
                       @endforeach
                   @endif  
                   <div class="flex flex-col gap-2">
                       ...
                       <div class="flex items-center gap-4 border border-main p-2 rounded-md w-28">
                           <img class="w-6" src="{{asset("image/time.png")}}" alt="">
                           <input type="text" name="time" id="time" value="{{$memory->time??"00:00"}}" placeholder="00:00" pattern="[0-9]{2}:[0-9]{2}" class="bg-main text-main focus:outline-none text-sm placeholder:text-secondary w-full"> 
                       </div>
                   </div>
                   <button type="submit" class="uppercase px-8 py-3.5 font-bold rounded-lg w-full lg:w-auto block text-xs bg-button">Edit Game</button>
                </div>
            </form>
        </div>
    </div>
